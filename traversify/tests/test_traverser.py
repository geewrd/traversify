...
class UpdativeTests(unittest.TestCase):
    ...
    def test_setitem_updates_nested_attributes_dict_correctly(self):
        obj = Traverser({'parent': {'child': 'orig_value'}})
        obj.parent.child = 'new_value'
        self.assertEqual(obj.parent.child, 'new_value')

    def test_call_updates_attributes(self):
        obj = Traverser({'parent': {'child': [None, {'orig_attr': 'orig_value'}]}})
        obj('parent.child.0', True)
        self.assertTrue(obj.parent.child[1])

    def test_call_overrides_attributes(self):
        obj = Traverser({'parent': {'child': [None, {'orig_attr': 'orig_value'}]}})
        obj('parent.child.baby', True)
        self.assertTrue(obj.parent.child.baby)
        obj('parent.child.0', False)
        self.assertFalse(obj.parent.child[0])
        obj('parent', 123)
        self.assertEqual(obj.parent, 123)

    def test_call_returns_updated_obj(self):
        obj = Traverser({'parent': {'child': [None, {'orig_attr': 'orig_value'}]}})
        obj('parent.child.baby', True)
        self.assertTrue(Traverser({'parent': {'child': {'baby': True}}}))

    def test_call_with_index_appends_or_inserts_if_index_out_of_bound(self):
        obj = Traverser({'parent': {'child': []}})
        obj('parent.child.0', 'append')
        self.assertEqual(obj.parent.child[0], 'append')
        obj('parent.child.-3', 'insert')
        self.assertEqual(obj.parent.child[0], 'insert')
